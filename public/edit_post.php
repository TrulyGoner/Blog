...
if (!$post_data || $post_data['user_id'] != $current_user['id']) {
    flash_message('Пост не найден или у вас нет прав на его редактирование', 'error');
    redirect('index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            $result = $post->update($post_id, $current_user['id'], $title, $content);
            if ($result['success']) {
                flash_message('Пост успешно обновлен!', 'success');
    ...
    $_POST['title'] = $post_data['title'];
    $_POST['content'] = $post_data['content'];
}
...
<head>
    ...
    <title>Редактировать пост - <?php echo SITE_NAME; ?></title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center space-x-4">
                    <a href="post.php?id=<?php echo $post_id; ?>" class="text-gray-700 hover:text-blue-600">
                        <i class="fas fa-arrow-left mr-1"></i>Назад к посту
                    </a>
